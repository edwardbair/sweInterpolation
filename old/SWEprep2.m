function [SWE,P,C,XP,XC]=SWEprep2(mstruct,R,D,Z,S,pflag,cflag,conn)
...
[P,C,XP,XC]=getGridStations2(mstruct,R,Z,conn);
...
cswe=[];
if pflag
    [pswe,pillowdates]=getSWE2(P,'pillow',...
...
if cflag
    [cswe,coursedates]=getSWE2(C,'course',...
        datestr(min(D)-.5),datestr(max(D)+.5),conn);
    if ~isempty(cswe)
        cswe(:,2)=-cswe(:,2);
